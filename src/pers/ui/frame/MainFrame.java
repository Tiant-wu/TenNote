...
public class MainFrame extends AbstractFrame
{
	...
	private JButton btnBound, btnItalic, btnUnderline,btnLine,btnLeft,btnCenter,btnRight,btnColor,btnPicture,btnEmoji;
	...
	private void initUI()
	{
		...
		btnBound = new JButton(new ImageIcon(MainFrame.class.getResource("/res/加粗.png")));
		btnItalic = new JButton(new ImageIcon(MainFrame.class.getResource("/res/倾斜.png")));
		btnUnderline = new JButton(new ImageIcon(MainFrame.class.getResource("/res/下划线.png")));
		btnColor = new JButton(new ImageIcon(MainFrame.class.getResource("/res/文字颜色.png")));
		btnLine= new JButton(new ImageIcon(MainFrame.class.getResource("/res/文字横线.png")));
		btnLeft= new JButton(new ImageIcon(MainFrame.class.getResource("/res/左对齐.png")));
		btnCenter= new JButton(new ImageIcon(MainFrame.class.getResource("/res/居中对齐.png")));
		btnRight= new JButton(new ImageIcon(MainFrame.class.getResource("/res/右对齐.png")));
		btnPicture= new JButton(new ImageIcon(MainFrame.class.getResource("/res/插图.png")));
		...
		//新建-目录
		fileMenu_NewDir = new JMenuItem("新建目录");
		fileMenu_NewDir.addActionListener(this);
		fileMenu.add(fileMenu_NewDir);
		...
		treePopupMenu_DeleteChildDir = new JMenuItem("删除子目录");
		treePopupMenu_DeleteChildDir.addActionListener(this);
		treePopupMenu.add(treePopupMenu_DeleteChildDir);
		
		//添加侧边栏
		loadTree();
		if(null == tree) {
		    root = new DefaultMutableTreeNode("TenNote");
	...
	private void createNewDir() {
	    String str = JOptionPane.showInputDialog(this,"输入目录：","新建目录",JOptionPane.PLAIN_MESSAGE);
	    if(str != null) {
	    	
	    	DefaultMutableTreeNode newDir = new DefaultMutableTreeNode(str);
	    	model.insertNodeInto(newDir,root,root.getChildCount());
	    	
	    	TreeNode[] treeNode=model.getPathToRoot(newDir);
	 ...
	 * 新建子目录
	 */
	private void createNewChildDir() {
		String str = JOptionPane.showInputDialog(this,"输入目录：","新建目录",JOptionPane.PLAIN_MESSAGE);
	    if(str != null) {
	    	DefaultMutableTreeNode newChildDir = new DefaultMutableTreeNode(str);
	    	
	    	DefaultMutableTreeNode select=(DefaultMutableTreeNode)tree.getLastSelectedPathComponent();
            if(select==null) return;
            model.insertNodeInto(newChildDir, select, select.getChildCount());
            
            TreeNode[] treeNode=model.getPathToRoot(newChildDir);
            TreePath treePath=new TreePath(treeNode);
            tree.scrollPathToVisible(treePath);
            validate();
